...
def NB_PCA (k,baseline_accuracy, X_train, X_test, Y_train, Y_test) :
    ...
    accs2 , cm2 , cr2 = nb2.evaluate (Final_result_XTrain , Y_train ,Final_result_XTest, Y_test)
...
def get_baseline_categorical (X_train_cat, X_test_cat, Y_train_cat, Y_test_cat) :
    ...
    accs3 , cm3 , cr3 = nb3.evaluate (X_train_encoded, Y_train_cat,X_test_encoded,  Y_test_cat)

def NBA_PCA_categorical (k,baseline_accuracy,X_train_cat, X_test_cat,Y_train_cat, Y_test_cat) :
    X_train_encoded = Encoding_categorical_data (X_train_cat)
    X_test_encoded = Encoding_categorical_data (X_test_cat)
